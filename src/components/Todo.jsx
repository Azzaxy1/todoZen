  ...
  return data.map((dataApp, i) => {
    return (
      <div
        ...
        key={i}
      >
        <p id={`taskName${dataApp.id}`}>{dataApp.task}</p>
        <div className="flex gap-2">
          <input
            type="checkbox"
            onChange={() => changeCompleted(dataApp.id)}
            checked={dataApp.completed}
          />
          <FontAwesomeIcon
            icon={faPenToSquare}
            onClick={() => editTodo(dataApp.id)}
          />
          <FontAwesomeIcon
            icon={faTrash}
            onClick={() => deleteTodo(dataApp.id)}
          />
        </div>
      </div>
    );
